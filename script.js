...
// lat/lon coordinates into screen coordinates
  ...
  d3.json('https://d3js.org/world-50m.v1.json', function(error, data) {
    ...
    d3.json('coordinate.json', function(error, data) {
      if (error) 
        console.error(error);
      ...
      g.selectAll('circle')
        ...
        .attr('r', function(d) {
          if (d.properties.mass) {
            return Math.pow(parseInt(d.properties.mass), 1 / 2.5);
          }
        })
        .style('fill', 'red')
        .style('stroke', 'black')
      ...
      //Add Event Listeners | mouseover
        .on('mouseover', function(d) {
          ...
          d3.select('#Location').text(d.properties.Location);
          d3.select('#FirstName').text(d.properties.FirstName);
          d3.select('#LastName').text(d.properties.LastName);
